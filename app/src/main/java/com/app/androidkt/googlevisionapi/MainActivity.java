...
public class MainActivity extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    private static final String CLOUD_VISION_API_KEY = "AIzaSyDihT4Jxqlfq16Y5onkH2AycwnxrvSQ1dE";
    ...
    private String[] visionAPI = new String[]{"FACE_DETECTION", "LOGO_DETECTION", "SAFE_SEARCH_DETECTION", "IMAGE_PROPERTIES", "LABEL_DETECTION"};
    ...
    private String convertResponseToString(BatchAnnotateImagesResponse response) {
        ...
        switch (api) {
            case "FACE_DETECTION":
                List<FaceAnnotation> faceAnnotations = imageResponses.getFaceAnnotations();
                message = getFaceAnnotation(faceAnnotations);
                break;
    ...
    private String getFaceAnnotation(List<FaceAnnotation> faceAnnotations) {
        String message = "";

        if (faceAnnotations != null) {
            message = String.format("%s person(s) detected.", faceAnnotations.size());

            for (FaceAnnotation faceAnnotation : faceAnnotations) {
                String joyLikelihood = faceAnnotation.getJoyLikelihood();
                if(!joyLikelihood.contains("UNLIKELY")){
                    message = message + "\n" + "It looks joyful.";
                    break;
                }
            }
        } else {
            message = "No Human Found";
        }
        return message;
